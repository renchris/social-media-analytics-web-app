...
type ChartTooltipContentProps = TooltipBaseProps & Omit<React.ComponentProps<'div'>, 'children'>
...
const ChartStyle = ({ id, config }: ChartStyleProps) => {
  ...
  const generateThemeStyles = (theme: string, prefix: string) => {
    const colorDefinitions = colorConfig
      .map(([key, itemConfig]) => {
...
${colorDefinitions}
  ...
  const styles = Object.entries(THEMES)
    .map(([theme, prefix]) => generateThemeStyles(theme, prefix))
    .join('\n')
...
interface ChartLegendContentProps extends Omit<React.ComponentProps<'div'>, 'children'> {
  hideIcon?: boolean
  nameKey?: string
