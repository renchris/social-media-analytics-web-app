...
export function aggregateMetricsByDateAndBreakdowns(
  data: MetricData[],
  breakdowns: BreakdownType[],
): AggregatedMetric[] {
  const metrics = Object.keys(data[0]).filter(
    (key) => !['date_start', 'date_stop', 'gender'].includes(key),
  ) as MetricType[]

  const aggregated = data.reduce((acc, curr) => {
    const dateKey = `${curr.date_start}-${curr.date_stop}`
    const gender = breakdowns.includes('gender') ? (curr.gender || 'unknown') : 'unknown'
    const groupKey = `${dateKey}-${gender}`

    if (!acc[groupKey]) {
      acc[groupKey] = {
        date_start: curr.date_start,
        date_stop: curr.date_stop,
        ...(breakdowns.includes('gender') && { gender }),
    ...
    metrics.forEach((metric) => {
      if (curr[metric]) {
        acc[groupKey][metric] += Number(curr[metric])
      }
    })
